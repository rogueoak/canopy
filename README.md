...
override is a flat hex. A dark value equal to its light value is reported as a warning, not a
failure - sameness across themes is a design choice, and AA already proves it legible. A quick **runtime** path (redefining `--color-*` in your own `:root`/`.dark`)
...
full component library - Roots, Seeds, Twigs, and Branches - shipped in **1.0.0**; **1.5.0** is the
