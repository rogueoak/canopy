 ...
 * elevation token yet). `sideOffset` defaults to `4` to nudge the card off the trigger. The
 * `TooltipPrimitive.Arrow` is filled with `fill-surface-raised` so the SVG pointer matches the
 * card face in both themes.
 */
...
>(({ className, sideOffset = 4, children, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        'z-50 overflow-hidden rounded-md border border-border bg-surface-raised px-3 py-1.5 text-xs text-text shadow-md',
        className,
      )}
      {...props}
    >
